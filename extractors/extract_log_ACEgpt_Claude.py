...
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "logs_ACEgpt_Claude_results.csv")
...
def parse_user_and_week_from_path(path: str):
    ...
    Given a Firestore document path like:
        experiments/Experiment_ACEGPT/users/U01/weeks/Week1/logs/plan
...
def fetch_log_data(db):
    """
    Search across ALL 'logs' subcollections using collection_group('logs').

    For each doc:
      - Keep only /experiments/Experiment_ACEGPT/.../logs/plan
      - Read:
            Post_fat_pct
            Post_muscle_kg
            Post_weight_kg
    ...
    for idx, doc in enumerate(docs, start=1):
        ...
        print(
            ...
            f"feedback_present={free_text_feedback is not None} | notes_present={notes is not None}"
        )
...
def write_csv(rows):
    ...
    'daily_avg_kcal', 'sleep_avg_hours', 'free_text_feedback', 'notes'
    """
...
def main():
    print("=== Extract logs (Experiment_ACEGPT) to CSV ===")
    db = init_firestore()
    rows = fetch_log_data(db)

    if not rows:
        print("\n[RESULT] No rows were found containing the target log fields.")
        print("         Check that the fields exist in:")
        print("         /experiments/Experiment_ACEGPT/users/{user_id}/weeks/{week_number}/logs/plan")
    else:
        write_csv(rows)
