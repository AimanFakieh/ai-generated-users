...
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "logs_results.csv")
...
def parse_user_and_week_from_path(path: str):
    ...
    This function is robust as long as there is a 'users/{id}/weeks/{id}/logs/{doc}'
    pattern somewhere in the path.
    """
    segments = path.split("/")  # e.g. ['experiments', 'Experiment_OpenAI', 'users', 'U01', 'weeks', 'Week1', 'logs', 'plan']
...
def fetch_log_data(db):
    ...
    For every document under a 'logs' collection:
      ...
      - Check if it has any of these fields:
            Post_fat_pct
            Post_muscle_kg
            Post_weight_kg
            daily_avg_kcal
    ...
    # Stream all documents in any 'logs' collection
    ...
    for idx, doc in enumerate(docs, start=1):
        ...
        # We only want /.../logs/plan documents (as per your directory)
        segments = path.split("/")
        if not segments or segments[-1] != "plan":
            continue

        # Only consider logs under the Experiment_OpenAI experiment
        # (optional but matches your directory)
...
def main():
    print("=== Extract logs (multiple fields) from Firestore to CSV (collection_group logs) ===")
    db = init_firestore()
