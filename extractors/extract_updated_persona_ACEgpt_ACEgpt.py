...
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "updated_persona_ACEgpt_ACEgpt_results.csv")
...
def parse_user_and_week_from_path(path: str):
    ...
    Given a Firestore document path like:
        experiments/ACEGPT_ACEGPT/users/U01/weeks/Week1/updated_persona/plan

    Extract:
        user_id  = U01
        week_num = Week1
    """
    segments = path.split("/")  # e.g. ['experiments', 'ACEGPT_ACEGPT', 'users', 'U01', 'weeks', 'Week1', 'updated_persona', 'plan']
...
def main():
    print("=== Extract updated_persona (ACEGPT_ACEGPT) data from Firestore to CSV ===")
    db = init_firestore()
    ...
    if not rows:
        ...
        print("         /experiments/ACEGPT_ACEGPT/users/{user_id}/weeks/{week_number}/updated_persona/plan")
    else:
        write_csv(rows)
