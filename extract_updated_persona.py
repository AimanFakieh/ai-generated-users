...
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "updated_persona_results.csv")
...
def parse_user_and_week_from_path(path: str):
    ...
    Given a Firestore document path like:
        experiments/Experiment_OpenAI/users/U01/weeks/Week1/updated_persona/plan

    Extract:
        user_id  = U01
        week_num = Week1

    This function is robust as long as there is a 'users/{id}/weeks/{id}/updated_persona/{doc}'
    pattern somewhere in the path.
    """
    segments = path.split("/")  # e.g. ['experiments', 'Experiment_OpenAI', 'users', 'U01', 'weeks', 'Week1', 'updated_persona', 'plan']
...
def fetch_updated_persona_data(db):
    ...
    For every document under an 'updated_persona' collection:
      ...
      - Check for the target persona fields.
    ...
    for idx, doc in enumerate(docs, start=1):
        ...
        # We only want /.../updated_persona/plan documents (as per your directory)
        segments = path.split("/")
        if not segments or segments[-1] != "plan":
            continue

        # Only consider docs under the Experiment_OpenAI experiment
        if "experiments/Experiment_OpenAI" not in path:
            continue
        ...
        # Skip if none of the fields are present
        if all(
            value is None
            for value in [
...
def main():
    print("=== Extract updated_persona data from Firestore to CSV (collection_group updated_persona) ===")
    db = init_firestore()
    ...
    if not rows:
        ...
        print("         /experiments/Experiment_OpenAI/users/{user_id}/weeks/{week_number}/updated_persona/plan")
    else:
        write_csv(rows)
